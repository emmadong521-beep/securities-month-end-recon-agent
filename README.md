...
3. Run sample task: `请分析 2025-03 经纪佣金收入差异最大的异常，并定位根因。`
4. Review plan → tool calls → evidence chain → root-cause report.
5. Export validated monthly-close data.
...
Financial reconciliation requires reproducible calculations and traceable evidence. This project uses a hybrid design:

- Deterministic code handles amount calculation, reconciliation checks, allocation validation, severity grading, case matching, and evidence-chain construction.
