...
- `chart_of_accounts`：总账科目表，采用 1xxx 资产、2xxx 负债、3xxx 共同、
  4xxx 权益、5xxx 成本、6xxx 损益编码体系。
...
- `data_quality_report`：由 `python -m src.data_quality` 生成，包含行数、
  主键、外键、金额空值、金额勾稽、异常埋入与检测、公开指标校准和
  检查结论。
- `evidence_chain`：由 `src/evidence_chain.py` 动态生成，按异常编号展示
  收入链路或费用分摊链路的逐层证据。
